...
**Status:** Published as [`v1.25.13`](https://github.com/Alaa91H/Muslim/releases/tag/v1.25.13) after [tag CI](https://github.com/Alaa91H/Muslim/actions/runs/33084370719) completed successfully: quality, Android emulator, and signed release-artifact jobs passed. The phone `app-release.apk` SHA-256 (`71db5df5dbab59da67c5d37e70526003c18f2cc5e6656cae1dc9671e310c43ae`) and Wear `wear-release.apk` SHA-256 (`b8c1dbd537820fcb54b0db1951589bb156f298fbb87a06324b3bbff8e8d8d604`) were downloaded from the release and matched GitHub's published asset digests. This workspace still has no configured Android SDK, so its local evidence remains limited to static guards and Detekt.
...
|---|---|---|
| Hadith library | The catalogue is metadata-only. A user opening a collection imports that single book in bounded 150-row Room transactions; it supports its retained GZIP asset or Android’s unpacked NDJSON asset representation, and chapters and search stay scoped to the active book while Compose consumes Paging. Reader-facing recovery is localized and does not expose paths or exceptions. In Arabic UI, all 417 collection-scoped chapter labels have Arabic display titles while immutable source titles remain the Room/Paging/navigation keys. | Static asset, repository, DAO and UI guards pass locally; a unit test covers representative localized titles. Android memory profiling and device scrolling remain CI/device work. |
| Location | The picker accepts approximate or precise foreground permission. Precise access requests a high-accuracy current fix; a recent fused/platform fix is a local fallback. Fused-client and `LocationManager` construction are lazy, application-context, and recoverable before any request begins. Coordinates must resolve to an IANA zone before persistence. Play Services/OEM provider setup, geocoding, timezone, save, and rescheduling failures are contained at both provider and ViewModel boundaries in the recoverable UI state. | Code and static contract pass locally. Availability also depends on a user-enabled Android location provider and granted permission. |
...
| Live Adhan | While playback owns the foreground service, its public high-priority, ongoing, non-auto-cancelable notification remains in the drawer and lock screen. The sole action is Stop Adhan and it does not require authentication when invoked from the lock screen. Task removal re-posts the same card. Synthesized audio serializes every native `AudioTrack` operation through the same ownership/invalidation/release boundary, clearing ownership before release so a stale writer cannot reach Android’s invalid track state. | Android instrumentation verifies the card configuration and active-card posting. Lock-screen visibility and heads-up availability remain user/channel/OEM controlled. |
...
scripts/verify_hadith_chapter_localization.py
