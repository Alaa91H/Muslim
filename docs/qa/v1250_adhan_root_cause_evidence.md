...
## User-device evidence after v1.24.13

The supplied notification panel shows the persistent **Next Adhan** countdown in the silent-notifications section, not the active Adhan alert. Its retained full-colour artwork is consistent with an existing ongoing notification posted by an older build: Android keeps an ongoing notification across an APK update until the application explicitly cancels or replaces that notification id. The current source had no startup migration that cancels the retired countdown id before starting the refreshed service.

The Verify Adhan screen again reports that the active alert and audio start are not confirmed even though location, Adhan enablement, channel, exact alarms, and device alarm volume are reported as ready. In the current receiver, `startForegroundService()` can return without an exception while the system never delivers `onStartCommand()` (for example, a delayed or rejected background service start). The receiver then has no watchdog because it only performs direct playback when the start call itself throws. This leaves a gap where neither the service nor the direct fallback reaches the audio-start journal checkpoint.
