...
The screen exposes collection preparation progress, retry for a failed selected-book import, chapter indexes, and page load-state retry; readers receive a localized recovery message rather than an asset path or an implementation exception. All **417** bundled, collection-scoped chapter labels show the source-backed Arabic display title when the app UI is Arabic; the original source label remains the local Room/Paging/navigation key, and English/non-Arabic interfaces keep their existing label. The daily-Hadith worker does not trigger a hidden all-library import. The current versioned catalogue tracks **37,919** streamed records across Sahih al-Bukhari, Sahih Muslim, Sunan Abi Dawud, Jami at-Tirmidhi, Sunan an-Nasai, Sunan Ibn Majah, Muwatta Malik, Riyad as-Salihin and Forty Hadith of al-Nawawi. It is not a claim that all editions, translations, grading choices, or scholarly contexts are exhaustive. Religious-content and source review remain separate from software review. See [the lazy library and notification audit](docs/qa/lazy_hadith_location_notification_audit.md), the [cover provenance record](docs/content/hadith_cover_source_candidates.md), the [chapter-title localisation record](docs/content/hadith_chapter_title_localization.md), and the [official Paging overview](https://developer.android.com/topic/libraries/architecture/paging/v3-overview).
