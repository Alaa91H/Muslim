...
class HadithRepository @Inject constructor(
    ...
    suspend fun ensureSeeded() {
        ...
        seedMutex.withLock {
            if (seeded.get()) return
            val text = readSeedText(seedAsset())
    ...
    /** Whether the daily hadith notification is enabled (delegates to prefs). */
    ...
    /**
     * Reads the seed asset text. The full Six-Books corpus ships compressed
     * (`hadith_full.json.gz`, ~11 MB — the raw JSON is 54 MB and would bloat
     * both the repo and the APK), so it is transparently decompressed here;
     * the curated sample ships as plain JSON.
     */
    private fun readSeedText(asset: String): String = runCatching {
        if (asset.endsWith(".gz")) {
            java.util.zip.GZIPInputStream(context.assets.open(asset))
    ...
    /**
     * Uses the full Six-Books corpus (generated by scripts/import-hadith.py)
     * when bundled; falls back to the curated sample otherwise.
     */
    private fun seedAsset(): String = runCatching {
        context.assets.open(FULL_ASSET).close()
        FULL_ASSET
    }.getOrDefault(SAMPLE_ASSET)

    private companion object {
        const val SAMPLE_ASSET = "hadith_sample.json"
        const val FULL_ASSET = "hadith_full.json.gz"
    }
}
